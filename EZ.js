...
const chartIt = (
  ...
  ctxx = document.getElementById("c").getContext("2d"); //Just for now ...
...
const runModel = (modelName, input, CallBack= defultCallBack) => {
  ...
  if (modelName == "text to voice") {
    ...
    connect(GtApi, objectToSend, succsesCreateTone);
    ...
    connect(yoloApi2, objectToSend, sucssesYolo);
    ...
    connect(ivApi, objectToSend, succsesimageVar);
  }
  //sound file input
};
...
const base64ToImgTag = (Basse64, placeHolder = "body") => {
  const ph = elm(placeHolder);
  if (ph.length >= 1) {
